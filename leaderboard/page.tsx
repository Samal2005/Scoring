...
const LeaderboardPage: React.FC<LeaderboardProps> = ({ teams, sessions, config }) => {
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {filteredStats.map((team, idx) => (
                <tr key={team.id} className="hover:bg-white/[0.02] transition">
                  ...
                  <td className="px-8 py-8">
                    <span className="text-3xl font-mono font-bold text-white neon-text">{team.bestScore}</span>
                  </td>
                  <td className="px-8 py-8">
                    <span className="font-mono text-slate-400 font-bold">{team.bestTime > 0 ? `${(team.bestTime / 1000).toFixed(2)}s` : '--'}</span>
                  </td>
                </tr>
              ))}
            </tbody>
